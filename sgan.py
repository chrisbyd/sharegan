...
class SGAN(object):
    def __init__(self,hparams):
        ...
        self.logits_real = self.discriminator(self.Input_img)
    ...
    def generator(self):
        ...
        # a6=tf.nn.relu(tf.nn.conv2d_transpose(a5,W1_deconv,strides=[1,2,2,1],
        #                                      padding="SAME",
        #                                      output_shape=[tf.shape(a5)[0],14,14,64])+b1_deconv)
        # a7=tf.layers.batch_normalization(a6,training=True)
        # a8=tf.nn.tanh(tf.nn.conv2d_transpose(a7,W2_deconv,strides=[1,2,2,1],
        #                                      padding="SAME",
    ...
    def discriminator(self,x):
        ...
        # a1=tf.nn.leaky_relu(tf.nn.conv2d(x,W1_conv,strides=[1,1,1,1],padding='VALID')+b1_conv)
        # a2=tf.nn.max_pool(a1,ksize=[1,2,2,1],strides=[1,2,2,1],padding="VALID")
        # a3=tf.nn.leaky_relu(tf.nn.conv2d(a2,W2_conv,strides=[1,1,1,1],padding="VALID")+b2_conv)
