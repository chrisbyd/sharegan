...
def get_discriminator(img,data_cfg):
    with tf.variable_scope("discriminator", reuse=True):
        ...
        W2 = tf.get_variable("W2")
        b2 = tf.get_variable("b2")
    x = tf.reshape(img, [-1, data_cfg.dataset.image_size,data_cfg.dataset.image_size, data_cfg.dataset.channels])
    da1 = tf.nn.leaky_relu(
        tf.nn.conv2d(x, W1_conv, strides=[1, 2, 2, 1], padding='SAME') + b1_conv)  # [-1,16,16,64]
    da2 = tf.nn.leaky_relu(
        tf.nn.conv2d(da1, W2_conv, strides=[1, 1, 1, 1], padding='SAME') + b2_conv)  # [-1,8,8,128]
    da3 = tf.layers.batch_normalization(da2)
    da4 = tf.reshape(da3, [-1, 8 * 8 * 64])
