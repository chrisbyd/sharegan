...
def share_cat_loss(dis_cat_layer, gen_cat_layer):
    ...
    :return:
    """
    return tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=gen_cat_layer, labels=dis_cat_layer))
...
def get_solvers(learning_rate=1e-3, beta1=0.5):
    ...
    return D_solver, G_solver, C_solver
...
def generator(z):
    ...
    """
    with tf.variable_scope("generator1-cate"):
        # TODO: implement architecture
        ...
        W2 = tf.get_variable('W2', shape=[1024, 8 * 8 * 128])
        b2 = tf.get_variable('b2', shape=8 * 8 * 128)

        # w3 mapping noise to category
        W3 = tf.get_variable('W3', shape=[1024, CATGORY_INFO])
        b3 = tf.get_variable('b3', shape=CATGORY_INFO)
        ...
        shared_category = tf.nn.relu(tf.matmul(a2, W3) + b3)

    with tf.variable_scope("generator1-ref"):

        # mapping category to graph
        W4 = tf.get_variable('W3', shape=[CATGORY_INFO, 8 * 8 * 128])
        b4 = tf.get_variable('b3', shape=8 * 8 * 128)
        ...
        a4x = tf.layers.batch_normalization(tf.matmul(shared_category, W4) + b4, training=True, name="share")

        a5 = tf.reshape(a4 + a4x, [-1, 8, 8, 128])
        ...
        return img, shared_category


def run_a_gan(sess, G_train_step, G_loss, D_train_step, D_loss,  C_loss, C_train_step,
    ...
    for it in range(500):
        ...
        if it % print_every == 0:
            print('Iter: {}, D: {:.4}, G:{:.4}, C:{:.4}'.format(it, D_loss_curr, G_loss_curr, C_loss_cur))
    print('Final images')
...
# generated images
G_sample, real_gen_cat_layer = generator(z)
...
with tf.variable_scope("") as scope:
    ...
    logits_fake, dis_cat_layer_fake = discriminator(G_sample)
...
G_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'generator1-ref')
D_solver, G_solver, C_solver = get_solvers()
D_loss, G_loss = gan_loss(logits_real, logits_fake)
C_loss = share_cat_loss(dis_cat_layer_real, real_gen_cat_layer)
D_train_step = D_solver.minimize(D_loss, var_list=D_vars)
G_train_step = G_solver.minimize(G_loss - C_loss, var_list=G_vars)
C_train_step = C_solver.minimize(C_loss, var_list=C_vars)


with get_session() as sess:
    sess.run(tf.global_variables_initializer())
    run_a_gan(sess,G_train_step, G_loss, D_train_step, D_loss, C_loss, C_train_step)
